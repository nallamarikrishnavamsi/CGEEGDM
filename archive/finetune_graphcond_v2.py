...
class PrintEpochCallback(pl.Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        pass  # metrics not yet logged here (LightningModule hook runs after callbacks)

    def on_train_epoch_start(self, trainer, pl_module):
        # Print previous epoch's final validation metrics (now guaranteed fresh)
        ...
        print(f"[Epoch {epoch}] val/loss={val_loss:.4f} val/kappa={val_kappa:.4f} val/bacc={val_bacc:.4f}", flush=True)
...
class PLGraphConditionedClassifier(pl.LightningModule):
    ...
    def _shared_step(self, batch, use_ema=False):
        ...
        warmup_alpha  = min(1.0, (self.current_epoch + 1) / warmup_epochs) if warmup_epochs > 0 else 1.0
        logits, z_token, z_graph = model(
            (signal, None), icoh_vec, return_alignment=True, warmup_alpha=warmup_alpha
        )
        ...
        if z_token is None or self.current_epoch < self.hparams.get('align_start_epoch', 5):
    ...
    def configure_optimizers(self):
        # Separate param groups with independent base lr AND peak max_lr:
        #   backbone (pretrained, matches original EEGDM) -> lr=1e-4, max_lr=1e-3, wd=0.05
        #   graph modules (new, randomly init'd)          -> lr=2e-4, max_lr=2e-3, wd=0.10
        backbone_params = [p for p in self.model.get_backbone_params() if p.requires_grad]
        new_params      = [p for p in self.model.get_new_params() if p.requires_grad]
        backbone_lr     = self.hparams['opt_kwargs'].get('lr', 1e-5)
        backbone_max_lr = self.hparams['sch_kwargs'].get('backbone_max_lr', 5e-4)
        backbone_wd     = self.hparams['opt_kwargs'].get('weight_decay', 0.05)
        graph_lr        = self.hparams['opt_kwargs'].get('graph_lr', 1e-5)
        graph_max_lr    = self.hparams['sch_kwargs'].get('graph_max_lr', 5e-4)
        graph_wd        = self.hparams['opt_kwargs'].get('graph_weight_decay', 0.05)
        ...
        if backbone_params:
            ...
            max_lr_list.append(backbone_max_lr)
        if new_params:
            param_groups.append({'params': new_params, 'lr': graph_lr, 'weight_decay': graph_wd})
            max_lr_list.append(graph_max_lr)
        opt = torch.optim.AdamW(param_groups, betas=betas)
        sch = torch.optim.lr_scheduler.OneCycleLR(
            opt,
            max_lr=max_lr_list,
            total_steps=self.hparams['sch_kwargs']['total_steps'],
            pct_start=self.hparams['sch_kwargs'].get('pct_start', 0.2),
        )
        return [opt], [{'scheduler': sch, 'interval': 'step'}]

    def freeze_all_except_classifier_head(self):
        """
        Phase 2: freeze extractor, reducer, decoder, graph modules —
        train only the final TransformerClassifier head on top of the
        already-graph-conditioned representations learned in Phase 1.
        """
        for p in self.model.parameters():
            p.requires_grad = False
        for p in self.model.classifier.classifier.parameters():
            p.requires_grad = True
        n_trainable = sum(p.numel() for p in self.parameters() if p.requires_grad)
        n_total     = sum(p.numel() for p in self.parameters())
        print(f"Phase 2 freeze: {n_trainable:,} / {n_total:,} params trainable "
              f"(classifier head only)")

    def configure_optimizers_phase2(self, lr=1e-4, weight_decay=0.01, total_steps=1000):
        """
        Fresh optimizer/scheduler for the classifier-head-only Phase 2 pass.
        Only parameters with requires_grad=True (the head, after freezing)
        are included — matches freeze_all_except_classifier_head().
        """
        head_params = [p for p in self.model.classifier.classifier.parameters()
                      if p.requires_grad]
        opt = torch.optim.AdamW(head_params, lr=lr, weight_decay=weight_decay,
                                betas=(0.9, 0.98))
        sch = torch.optim.lr_scheduler.OneCycleLR(
            opt, max_lr=lr * 5, total_steps=total_steps, pct_start=0.2,
        )
        return opt, sch
...
def main(args):
    ...
    model = PLGraphConditionedClassifier(
        ...
        opt_kwargs   = dict(
            lr=1e-5, weight_decay=0.05,                          # backbone: verified original EEGDM values (base_*.yaml)
            graph_lr=args.graph_lr, graph_weight_decay=args.graph_weight_decay,
            betas=[0.9, 0.98],
        ),
        sch_kwargs   = dict(
            backbone_max_lr=5e-4,                                 # backbone: verified original EEGDM peak (base_*.yaml)
            graph_max_lr=args.graph_max_lr,
            total_steps=steps_per_epoch*args.epochs,
            pct_start=0.1,                                        # verified original EEGDM value (base_*.yaml)
        ),
        n_class      = 6,
        lambda_align = args.lambda_align,
        use_graph    = args.use_graph,
        backbone_ckpt = args.backbone_ckpt,
        align_start_epoch = args.align_start_epoch,
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ...
            pl.callbacks.EarlyStopping(monitor='val/loss', mode='min', patience=args.patience),
            PrintEpochCallback(),
        ]
    )
    if args.resume_phase1_ckpt:
        print(f"\n=== Skipping Phase 1 training — resuming from {args.resume_phase1_ckpt} ===")
        best = PLGraphConditionedClassifier.load_from_checkpoint(
            args.resume_phase1_ckpt, weights_only=False)
        print("\n=== Phase 1 test (loaded from existing checkpoint) ===")
        results_p1 = trainer.test(best, test_loader)
        print(results_p1)
    else:
        ...
        print("\n=== Phase 1 test (before classifier-head-only phase 2) ===")
        results_p1 = trainer.test(best, test_loader)
        print(results_p1)

    if args.phase2_epochs > 0:
        print(f"\n=== Phase 2: classifier-head-only finetune ({args.phase2_epochs} epochs) ===")
        best.freeze_all_except_classifier_head()

        phase2_steps = steps_per_epoch * args.phase2_epochs
        opt2, sch2 = best.configure_optimizers_phase2(
            lr=args.phase2_lr, weight_decay=args.phase2_weight_decay,
            total_steps=phase2_steps
        )
        best.configure_optimizers = lambda: ([opt2], [{'scheduler': sch2, 'interval': 'step'}])

        ckpt_dir_p2 = f"{ckpt_dir}_phase2"
        ...
        trainer_p2 = pl.Trainer(
            ...
            gradient_clip_val=3,
            enable_progress_bar=False,
            default_root_dir=f'logs/{args.name}_phase2',
            callbacks=[
                pl.callbacks.ModelCheckpoint(monitor='val/loss', mode='min', save_top_k=1,
                                             dirpath=ckpt_dir_p2, filename='best', save_last=True),
                PrintEpochCallback(),
            ]
        )
        trainer_p2.fit(best, train_loader, val_loader)

        best_p2_ckpt = trainer_p2.checkpoint_callbacks[0].best_model_path
        best_p2 = PLGraphConditionedClassifier.load_from_checkpoint(best_p2_ckpt, weights_only=False)

        print("\n=== Phase 2 test (classifier-head-only finetune result) ===")
        results_p2 = trainer_p2.test(best_p2, test_loader)
        print(results_p2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--align_start_epoch', type=int, default=5)
    parser.add_argument('--warmup_epochs', type=int, default=5)
    parser.add_argument('--graph_lr', type=float, default=1e-5)
    parser.add_argument('--graph_max_lr', type=float, default=5e-4)
    parser.add_argument('--graph_weight_decay', type=float, default=0.05)
    ...
    parser.add_argument('--phase2_weight_decay', type=float, default=0.01)
    parser.add_argument('--resume_phase1_ckpt', type=str, default=None,
                        help='Path to existing Phase 1 best.ckpt — skip Phase 1 training entirely')
