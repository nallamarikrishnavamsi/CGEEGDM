...
def main(config: DictConfig):
    ...
    _data_inst = hydra.utils.instantiate(config.finetune.data)
    if isinstance(_data_inst, dict):
        data_cfg = _data_inst
    else:
        data_cfg = OmegaConf.to_container(_data_inst, resolve=True)
    ...
    model = PLClassifierConnectivity(
        ...
        classifier_model_kwargs  = model_cfg['model_kwargs'],
    ...
    exp_name = os.path.basename(os.path.dirname(model_cfg['pretrain_checkpoint']))
    ft_ckpt_dir = f'checkpoint/finetune/{exp_name}'
    ...
    ]  # No early stopping in phase1 — run full phase1_epochs

    wandb_logger = WandbLogger(
        project="eegdm_graphcond",
        name=f"B0_phase1",
        entity="krishkrishnavamsi01-indian-institute-of-technology-kanpur",
    )
    ...
    # Update scheduler steps directly on instance vars
    model.sch_total_steps = steps_p2
    model.sch_max_lr = model_cfg['sch_kwargs'].get('max_lr', 1e-4)
    ...
    callbacks_p2 = [
        ...
        pl.callbacks.EarlyStopping(
            monitor='val/kappa', mode='max', patience=10
        ),  # patience=10 gives full 30 epochs chance to converge
        pl.callbacks.LearningRateMonitor(logging_interval='step')
    ]

    wandb_logger_p2 = WandbLogger(
        project="eegdm_graphcond",
        name=f"B0_phase2",
        entity="krishkrishnavamsi01-indian-institute-of-technology-kanpur",
    )
